# Pack plugin/ into a .ccx (a .ccx is just a zip of the plugin folder, renamed).
# Output: uxp-hybrid\dist\调色盘Hybrid.ccx
$ErrorActionPreference = "Stop"
$root   = "C:\Users\ccdragon\_code\ps_plugin\uxp-hybrid"
$plugin = Join-Path $root "plugin"
$dist   = Join-Path $root "dist"
$ccx    = Join-Path $dist "ColorPaletteHybrid.ccx"

# sanity: plugin must contain manifest + the addon it references
if (-not (Test-Path (Join-Path $plugin "manifest.json"))) { throw "manifest.json not found in plugin/" }
