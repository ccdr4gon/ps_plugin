...
let lastHue = -1;   // 缓存：SV 方块底色只在 hue 真变时重绘（大方块整体重绘最贵，是取色卡的主因）
function renderPicker() {
  if (state.h !== lastHue) {                 // hue 没变就不动 200px 大方块 → 大幅减少 UXP paint 积压
    lastHue = state.h;
...
function updateSwatchColors() {   // 前景色块（轻，跟手）
  ensureSwEls();
  swEls.fg.style.background = rgbStr(state.r, state.g, state.b);
}
...
let armed = false;          // 是否已通过「双击 Alt」激活吸色（防单击 Alt/Alt+Tab 误触）
...
let frame = 0;
let renderN = 0, renderFastN = 0, glSum = 0, glN = 0, glMax = 0, pollN = 0, pushN = 0;   // 临时诊断
let gapMax = 0, gapN = 0, lastTick = 0;   // 测每帧实际间隔（卡 = 间隔变大）
let needPush = false, lastPushFrame = -99;   // 待写前景色 + 上次写的帧（节流写 PS，防频繁 executeAsModal 阻塞）
setInterval(() => { console.log(`[perf] gap(max=${gapMax}ms n=${gapN}) getLatest(avg=${glN ? (glSum / glN).toFixed(1) : 0}ms max=${glMax}ms) fast/s=${renderFastN} push/s=${pushN} armed=${armed} picking=${picking}`); renderN = 0; renderFastN = 0; glSum = 0; glN = 0; glMax = 0; pollN = 0; pushN = 0; gapMax = 0; gapN = 0; }, 1000);
...
function sampleLoop() {
  ...
  try {
    ...
    // —— 双击 Alt 激活吸色（防单击 Alt / Alt+Tab 误触）——
    // 已激活(armed)后：完全忽略后续 Alt 上升沿（第3、4下连按的抖动不再反复触发判定 → 不卡）。
    if (!armed && alt && !prevAlt) {             // 仅未激活时检测双击的上升沿
      if (frame - lastAltDownFrame <= DOUBLE_FRAMES) armed = true;   // 距上次按下够近 = 双击 → 激活
      lastAltDownFrame = frame;
    }
    // armed 只在 Alt「彻底松开」后解除（连续 ~10 帧没按 Alt）；去抖避免瞬时抖动误清零
    if (alt) altFalse = 0;
    else if (++altFalse >= 10) { armed = false; lastAltDownFrame = -999; }
    prevAlt = alt;

    // 取色态：已双击激活(armed) + 按住 Alt + PS 在最前(#1) + 没点左键 + 没按 Ctrl/Shift/Tab(#3) + 没按 Esc。
    ...
    if (pickActive) {               // 取色态：双击 Alt 激活后，按住 Alt 划过画布（不点左键）
    ...
    if (picking) {                         // pickActive 抖一两帧(如 psActive 偶发误判)不算结束
      if (++pickFalse >= 12) {             // 连续 ~12 帧(>180ms) pickActive=false 才算真松开 Alt
        ...
        setStatus("就绪：双击 Alt 后按住划过画布取色", "ready");
      }
    ...
    if (frame % 6 === 0) { pollN++; pollForeground(); }  // 非取色：镜像 PS 前景色（降频~16fps，避免每帧读 PS 卡顿）
  } catch (e) {
...
function rafLoop() {
  frame++;
  const _now = Date.now();          // 测帧间隔：正常~16ms，卡时飙高
  if (lastTick) { const gap = _now - lastTick; gapN++; if (gap > gapMax) gapMax = gap; }
  lastTick = _now;
  ...
  // 取色态(armed/picking)每帧读屏；空闲时每 4 帧读一次（只为检测 Alt 双击，降低常驻同步读屏的全局卡顿）
...
async function startSampler() {
  ...
  try {
    sampler = await require("ColorPaletteSampler6.uxpaddon");
    console.log("[addon] Sampler6 加载成功; getLatest=" + (typeof sampler.getLatest) + " breakMenu=" + (typeof sampler.breakMenu));
    setStatus("就绪：双击 Alt 后按住划过画布取色", "ready");
  } catch (e) {
    sampler = null; addonErr = String(e);
    console.log("[addon] Sampler6 加载失败 → 退回镜像前景色: " + addonErr);
